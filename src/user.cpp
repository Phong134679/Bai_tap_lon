...
User::User(const string& uname, const string& pwd, bool isAuto, const string& mail)
    : username(uname), password(pwd), email(mail), isAutoGenerated(isAuto) {}
...
bool User::registerToDB(Database& db) {         //mã hóa và insert
    ...
    return db.insertUser(username, hashed, isAutoGenerated, email);
}
